...
describe Adherent::AdhesionsController, :type => :controller do
  ...
  describe 'GET new' do
    ...
    it 'assigne une adhésion et rend la vue new' do
      expect(@member).to receive(:next_adhesion).and_return(@adh = mock_model(Adherent::Adhesion))
  ...
  describe "GET edit" do
    it 'appelle le payement' do
     expect(@member).to receive(:adhesions).and_return(@ar = double(Arel)) 
      expect(@ar).to receive(:find).with('1').and_return(@adh = mock_model(Adherent::Adhesion))
  ...
  describe "POST create" do
    ...
    it 'crée une nouvelle adhésion avec les params' do
      expect(@member).to receive(:adhesions).and_return(@ar = double(Arel)) 
      expect(@ar).to receive(:new).with(@attrib).and_return(@adh = mock_model(Adherent::Adhesion))
